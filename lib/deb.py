...
class TileLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        print
        'TileLayer: name_to_top:', self._name_to_top_map
        assert len(top) == len(self._name_to_top_map)
...
class SiftFaceLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        print
        'SiftFaceLayer: name_to_top:', self._name_to_top_map
        assert len(top) == len(self._name_to_top_map)
    ...
    def forward(self, bottom, top):
        ...
        if np.all(np.unique(gt_mask_fg) == 1):
            ...
            dets = dets[
                keep]  # shape(n,5) n means n predictes boxes, 5 includes top left and bottom right coords and a score
            # enlarge boxes
            # dets[:,:4] *= 1.1
            #    print(dets)
            #    print(dets.shape)
...
class ShuffleMaskLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        top[0].reshape(bottom[0].data.shape[0], self._channels, bottom[0].data.shape[2], bottom[0].data.shape[3])
        print
        'ShuffleMaskLayer: name_to_top:', self._name_to_top_map
        assert len(top) == len(self._name_to_top_map)
...
class MaskPredLossLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        top[0].reshape(1)
        print
        'MaskPredLossLayer: name_to_top:', self._name_to_top_map
        assert len(top) == len(self._name_to_top_map)
...
class MaskGenLayer(caffe.Layer):
    ...
    def backward(self, top, propagate_down, bottom):
        top_diff = np.zeros(
            (top[1].diff.shape[0], top[1].diff.shape[1], top[1].diff.shape[2] / 3, top[1].diff.shape[3] / 3))
...
class SumLossLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        top[0].reshape(1)
        print
        'SumforLossLayer: name_to_top:', self._name_to_top_map
        assert len(top) == len(self._name_to_top_map)
