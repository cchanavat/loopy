...
class Model:
    ...
    def is_loop(self):
        axioms = [
            Axiom(f"Ax x*{self.identity_repr} = x", lang=self.lang),
            Axiom(f"Ax {self.identity_repr}*x = x", lang=self.lang),
            Axiom(r"Ax Ay x\(x*y) = y", lang=self.lang),
            Axiom(r"Ax Ay x*(x\y) = y", lang=self.lang),
            Axiom(r"Ax Ay (x*y)/y = x", lang=self.lang),
            Axiom(r"Ax Ay (x/y)*y = x", lang=self.lang),
        ]
        for ax in axioms:
            if not self.truth_value(ax):
                return False
        return True
